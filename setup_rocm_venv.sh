...
Options:
  ...
  --constraints <path> Constraints output path (default: <venv>/rocm711-torch-constraints.txt)
  -h, --help           Show help
USAGE
}
...
[[ -n "${CONSTRAINTS_PATH}" ]] || CONSTRAINTS_PATH="${VENV_DIR}/rocm711-torch-constraints.txt"
...
# Generated by rocm711_torch_example/setup_rocm_venv.sh
${NUMPY_SPEC}
...
if [[ "${1:-}" == "install" ]]; then
  shift
  exec "${VIRTUAL_ENV}/bin/python" -m pip install -c "${VIRTUAL_ENV}/rocm711-torch-constraints.txt" "$@"
fi
...
echo "  \"${pip_wrapper}\" install -r requirements.txt"
